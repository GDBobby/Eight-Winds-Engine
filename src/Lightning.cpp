...
namespace EWE {
	...
	void LightningSystem::update(lab::mat4* swordMatrix, float rotation, std::array<float, 3>& startingTranslation, lab::vec3& secondTranslation, std::array<float, 3>& finalTranslation) {
		lab::vec3 startT = lab::vec3(startingTranslation[0], startingTranslation[1], startingTranslation[2]);
		lab::vec3 secondP = startT;
		secondP.y += 4.f;
		lab::vec3 finalT = lab::vec3(finalTranslation[0], finalTranslation[1], finalTranslation[2]);
		finalT.y += 1.f;
		std::vector<lab::vec3*> tempVec{ &startT, & secondP, & secondTranslation, & finalT };
		...
		for (int i = 0; i < translations.size() - 1; i++) {
			...
			lab::vec3 startingPosition = translations[i][0];
			lab::vec3 endingPosition = translations[i + 1][0];
			lab::vec3 connector = (endingPosition - startingPosition) / static_cast<float>(translations[i].size());
			lab::vec3 theCross;
			if (i == 2) {
				theCross = lab::Normalized(lab::Cross(endingPosition - startingPosition, lab::vec3(1.f, 0.f, 0.f))) * 0.1f;
			}
			else {
	...
	void LightningSystem::update(const lab::mat4& startingMatrix, float rotation, std::vector<lab::vec3*>& transPoints) {
		...
		lab::vec3 tempRot = translations[0][0];
		tempRot.y = 0.f;
		tempRot = glm::rotate(lab::mat4(1.f), -rotation, lab::vec3(0.f, 1.f, 0.f)) * lab::vec4(tempRot, 1.f);
		tempRot.y = translations[0][0].y;
		...
		*/
		...
		for (int i = 0; i < transPoints.size() - 1; i++) { //i < 3
			...
			lab::vec3 startingPosition = translations[i][0];
			lab::vec3 endingPosition = translations[i + 1][0]; //up to 3
